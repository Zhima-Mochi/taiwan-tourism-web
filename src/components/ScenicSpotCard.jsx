...
const text_setting = "font-sans text-xs xl:text-sm font-semibold text-ellipsis whitespace-nowrap overflow-hidden w-5/6"
export default function ScenicSpotCard(props) {
    if (props.data) {
        return (
            <div key={props.data.ScenicSpotID} className="bg-white card btn shadow-xl">
                <img className="card-img" src={props.data.Picture.PictureUrl1 || df_img} alt="" />
                <div className="card-content flex flex-col justify-around py-2  px-4  word-color">
                    <h1 className="font-sans text-sm  xl:text-lg font-bold text-ellipsis whitespace-nowrap overflow-hidden">{props.data.ScenicSpotName}</h1>
